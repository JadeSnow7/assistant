...
from core.foundation.config_manager import config_manager, get_config
...
class UnifiedAPIGateway:
    ...
    async def update_configuration(self, path: str, value: Any) -> Dict[str, str]:
        """更新配置"""
        try:
            from core.foundation.config_manager import update_config
    ...
    async def get_configuration(self, path: str = None) -> Dict[str, Any]:
        """获取配置"""
        try:
            from core.foundation.config_manager import get_config
...
async def create_app(grpc_client=None) -> FastAPI:
    ...
    # 初始化配置管理器
    from core.foundation.config_manager import initialize_config_manager
